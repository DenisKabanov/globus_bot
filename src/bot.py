...
def start(update: Update, context: CallbackContext) -> None:
    ...
    if (chat_id not in db["chat_id"].values):
        new_user = pd.DataFrame({"user_name": [user_name], "chat_id": [chat_id], "current_country": [""], "current_answer": [""],
                                 "countries_history": [[]], "current_score": [0], "total_score": [0], "best_score": [0], "countries_hint": [{}]})
...
def clear_history(update: Update, context: CallbackContext):
    ...
    db.loc[db["chat_id"] == chat_id, "current_score"] = 0
    db.loc[db["chat_id"] == chat_id, "total_score"] = 0
    db.loc[db["chat_id"] == chat_id, "countries_hint"].iloc[0].clear()
...
def send_flag(update: Update, context: CallbackContext) -> None:
    ...
    db.loc[db["chat_id"] == chat_id, "current_score"] = 100
...
def answer_flag(update: Update, context: CallbackContext) -> None:
    ...
    current_score = db.loc[db["chat_id"] == chat_id, "current_score"].iloc[0]
    total_score = db.loc[db["chat_id"] == chat_id, "total_score"].iloc[0]
    best_score = db.loc[db["chat_id"] == chat_id, "best_score"].iloc[0]
    countries_hint = db.loc[db["chat_id"] == chat_id, "countries_hint"].iloc[0]
    ...
    if (answer_given.lower() == answer_expected.lower()) or \
       ...
       ((answer_expected == "Южно-Африканская Республика") and (answer_given.lower() == "юар")):
        ...
        update.message.reply_text(f"Поздравляю, страна {answer_expected} угадана! За отгадывание страны вы получили {current_score}. Итого {total_score}")
        ...
        new_total_score = total_score + max(0, current_score)
        db.loc[db["chat_id"] == chat_id, "total_score"] = new_total_score
        
        if new_total_score > best_score:
            db.loc[db["chat_id"] == chat_id, "best_score"] = new_total_score
                
        db.loc[db["chat_id"] == chat_id, "current_country"] = ""
        db.loc[db["chat_id"] == chat_id, "current_answer"] = ""
        db.loc[db["chat_id"] == chat_id, "countries_history"].iloc[0].append(answer_expected)
        db.loc[db["chat_id"] == chat_id, "countries_hint"].iloc[0].pop(answer_expected, None)
        
    else:
        
        current_score = current_score - 1
        db.loc[db["chat_id"] == chat_id, "current_score"] = current_score
        
        if answer_expected in countries_hint.keys():
            countries_hint[answer_expected] = countries_hint[answer_expected] + 1
        else:
           countries_hint[answer_expected] = current_score 
